...
def etl():
    ...
    bifm_cursor.execute("SELECT * FROM chotot_crawler WHERE is_extract = 0 LIMIT 50")
    ...
    if len(chotot_data_all) > 0:
        for chotot_data in chotot_data_all:
            try:
                ...
                # Update extract data  
                bifm_cursor.execute("UPDATE chotot_crawler SET is_extract = 1 WHERE id = " + str(chotot_data[0]))
                bifm_mysql.commit()
                print('--------------------------- Extract success DATA: ' + str(chotot_data[0]) + ' ---------------------------')
