...
def etl():
    ...
    bifm_cursor.execute("SELECT * FROM lazada_crawler WHERE is_extract = 0 LIMIT 50")
    ...
    if len(lazada_data_all) > 0:
        for lazada_data in lazada_data_all:
            ...
            if len(data_crawler['mods']['listItems']) > 0:
                ...
                # Update extract data  
                bifm_cursor.execute("UPDATE lazada_crawler SET is_extract = 1 WHERE id = " + str(lazada_data[0]))
                bifm_mysql.commit()
                print('--------------------------- Extract success DATA: ' + str(lazada_data[0]) + ' ---------------------------')
    etl()
