...
bifm_cursor.execute("SELECT * FROM shopee_crawler")
...
for shopee_data in shopee_data_all:
    ...
    if len(data_crawler['mods']['listItems']) > 0:
        for shopee_item in data_crawler['mods']['listItems']:
            name = shopee_item['name']
            price = shopee_item['price']
            description = None
            item_id = shopee_item['itemId']
            seller_id = shopee_item['sellerId']
            category_id = shopee_item['categories'][0]
            thumbs = shopee_item['thumbs'] if 'thumbs' in shopee_item else None
            ...
            if (len(check_data) == 0):
                ...
                bifm_cursor.execute("INSERT INTO product_shopee (name,price,description,item_id,seller_id,category_id) VALUES (%s,%s,%s,%s,%s,%s)", val)
                bifm_mysql.commit()
                ...
                if thumbs is not None:
                    for image in thumbs:
                        ...
                        image_path = 'C:/DSOFT/DATA/shopee/' + thumbnail_image
