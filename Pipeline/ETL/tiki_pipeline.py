...
def etl():
    ...
    bifm_cursor.execute("SELECT * FROM tiki_crawler WHERE is_extract = 0 LIMIT 50")
    ...
    if len(tiki_data_all) > 0:
        for tiki_data in tiki_data_all:
            ...
            if len(data_crawler['data']) > 0:
                ...
                # Update extract data
                bifm_cursor.execute("UPDATE tiki_crawler SET is_extract = 1 WHERE id = " + str(tiki_data[0]))
                bifm_mysql.commit()
                print('--------------------------- Extract success DATA: ' + str(tiki_data[0]) + ' ---------------------------')
    etl()
